...
function init() {
  ...
  renderer = new THREE.WebGLRenderer({ antialias: true, powerPreference: 'high-performance', depth: true });
  ...
  const material = new THREE.MeshStandardMaterial({ color: 0x44aaff, wireframe: false });
...
// scene.add(new THREE.DirectionalLight(0xffffff, 13));
  ...
  audioLoader.load('/audio/Midnight City Lights-Non Copyright Music.mp3', (buffer) => {
    sound.setBuffer(buffer);
    sound.setLoop(true);
    sound.setVolume(0.6);
    sound.play();

    analyser = new THREE.AudioAnalyser(sound, 64); // Change to a higher fftsize if any square grid doesnt move (e.g., 256, 128, upto 2048)
    ...
    document.getElementById('info').textContent = 'Failed to load audio. Using fallback animation.';
    animate(); 
  });
